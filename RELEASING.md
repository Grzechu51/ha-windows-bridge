...
```powershell
git status
git add .github custom_components ha_windows_bridge installer tests tools
...
```powershell
git tag -a v1.1.0 -m "HA Windows Bridge 1.1.0"
git push origin v1.1.0
```

Jeżeli tag `v1.1.0` już istnieje na złym commicie, usuń nieudane wydanie oraz tag na
GitHubie, usuń tag lokalny i utwórz go ponownie. Nie używaj force push do poprawiania
opublikowanego tagu.

## 4. Utworzenie Release

Na GitHubie otwórz **Releases → Draft a new release**, wybierz `v1.1.0`, ustaw tytuł
**HA Windows Bridge 1.1.0** i dołącz:

- `dist/HA-Windows-Bridge-Setup-1.1.0.exe`;
- `dist/HA-Windows-Bridge-1.1.0-win64.zip`;
- `dist/HA-Windows-Bridge-HA-Integration-1.1.0.zip`;
- `dist/SHA256SUMS-1.1.0.txt`.
