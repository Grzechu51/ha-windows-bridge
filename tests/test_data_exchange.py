...
def test_complete_configuration_export_has_no_removed_designer_data(tmp_path) -> None:
    path = tmp_path / "complete.json"
    export_configuration(path, AppConfig())

    payload = json.loads(path.read_text(encoding="utf-8"))["config"]

    assert "overlay_templates" not in payload
    assert "selected_overlay_template_id" not in payload
