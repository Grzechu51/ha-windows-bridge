...
def test_windows_media_overlay_includes_player_name_and_artwork() -> None:
    from ha_windows_bridge.discovery import overlay_notification_topic
    ...
    bridge._on_message(
        ...
        FakeMessage(
            ...
            b'{"title":"","message":"","data":{"action":"show","media":true}}',
            False,
        ),
    )

    assert len(overlays) == 1
    title, message, data = overlays[0]
    assert title == "Test track"
    assert message == "Test artist"
    assert data["layout"] == "media"
    assert data["media_source"] == "Office PC Media Player"
...
def test_power_action_and_notification_commands_are_dispatched() -> None:
    ...
    bridge._on_message(
        ...
        FakeMessage(
            overlay_notification_topic(config),
            b'{"title":"","message":"","data":{"action":"show","id":"light","layout":"badge","icon":"mdi:lightbulb-on"}}',
            False,
        ),
    )
...
def test_master_and_media_volume_are_not_artificially_limited() -> None:
    config = AppConfig(
        mqtt=MqttConfig(host="broker", base_topic="hawn/pc"),
        apps=[],
    )
    ...
    assert audio.master_set_calls == [0.9, 0.95]
    assert audio.output_calls == []
    assert audio.set_calls == []
