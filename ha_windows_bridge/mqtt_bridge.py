...
class MqttBridge:
    ...
    def _handle_overlay_notification(self, payload: bytes) -> None:
        ...
        image_requested = isinstance(data, dict) and bool(data.get("image") or data.get("qr"))
        ...
        if media_requested and action in {"show", "update"}:
            ...
            if snapshot.supported:
                ...
                data.update(
                    {
                        "layout": "media",
                        "media_source": f"{self.config.device_name or 'PC'} Media Player",
                        "icon": data.get("icon") or "mdi:cast-audio",
