...
def all_possible_discovery_messages(config: AppConfig) -> list[DiscoveryMessage]:
    ...
    # Keep the retired audio-profile entity in the cleanup catalogue so its
    # retained Discovery definition is removed on the first connection.
    legacy_profile_command, legacy_profile_state = audio_profile_topics(expanded)
    legacy_profile_payload = _base_entity(expanded, f"{object_root}_audio_profile")
    legacy_profile_payload.update(
        {
            "name": "Audio Profile",
            "command_topic": legacy_profile_command,
            "state_topic": legacy_profile_state,
            "options": ["Removed"],
        }
    )
    messages.append(
