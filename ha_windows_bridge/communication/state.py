...
class ConnectionMachine:
    ...
    def begin(self) -> int:
        with self._lock:
            self._epoch += 1
            epoch = self._epoch
            status = self._set(ConnectionState.CONNECTING)
        self.events.emit("connection.changed", status)
        return epoch
    ...
    def connected(self, epoch: int) -> bool:
        with self._lock:
            if epoch != self._epoch or self._status.state != ConnectionState.CONNECTING:
                return False
            status = self._set(ConnectionState.CONNECTED)
        self.events.emit("connection.changed", status)
        return True
    ...
    def failed(self, epoch: int, code: str, *, authentication: bool = False, configuration: bool = False) -> bool:
        with self._lock:
            ...
            status = self._set(state, self._status.attempt + 1, code)
        self.events.emit("connection.changed", status)
        return True
    ...
    def retry(self, epoch: int) -> bool:
        with self._lock:
            if epoch != self._epoch or self._status.state != ConnectionState.RETRY_WAIT:
                return False
            status = self._set(ConnectionState.CONNECTING, self._status.attempt)
        self.events.emit("connection.changed", status)
        return True
    ...
    def stop(self, *, suspended: bool = False) -> None:
        with self._lock:
            self._epoch += 1
            status = self._set(ConnectionState.SUSPENDED if suspended else ConnectionState.STOPPED)
        self.events.emit("connection.changed", status)

    def _set(self, state: ConnectionState, attempt: int = 0, error: str = "") -> ConnectionStatus:
